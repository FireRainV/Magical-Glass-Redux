...
function spell:onCast(user, target)
    ...
    Game.battle.timer:script(function(wait)
		...
		Game.battle.timer:every(0.01, function()
			local starparticle = Sprite("effects/stars/rainbow_star", Game.battle.starbasic.x + Utils.random(32), Game.battle.starbasic.y + Utils.random(32))
...
function spell:getDamage(user, target)
    if Game:isLight() then
        return math.ceil((user.chara:getStat("magic") * 10) + 55 + (Utils.random(5) * 2))
    else
        return math.ceil((user.chara:getStat("magic") * 20) + 100 + (Utils.random(10) * 2))
    end
end
