...
function spell:onCast(user, target)
	...
	Game.battle.timer:every(0.01, function(wait)
		...
		if random == 0 then
			starparticle = Sprite("effects/stars/starstorm_big", Utils.random(SCREEN_WIDTH), Utils.random(SCREEN_HEIGHT))
		elseif random == 1 then
			starparticle = Sprite("effects/stars/starstorm_medium", Utils.random(SCREEN_WIDTH), Utils.random(SCREEN_HEIGHT))
		else
			starparticle = Sprite("effects/stars/starstorm_small", Utils.random(SCREEN_WIDTH), Utils.random(SCREEN_HEIGHT))
		end
...
function spell:getDamage(user, target)
    if Game:isLight() then
        return math.ceil((user.chara:getStat("magic") * 10) + 70 + Utils.random(10))
    else
        return math.ceil((user.chara:getStat("magic") * 20) + 130 + (Utils.random(10) * 4))
    end
end
